...
namespace CollectiveMind.Ladybug.Runtime.Gameplay.Ladybug
{
  public class LadybugRotator : ILadybugRotator
  {
    ...
    public LadybugRotator(LadybugFacade facade, DrawingConfig config, LadybugConfig ladybugConfig)
    {
      _facade = facade;
      _config = config;
    ...
    {
      if (Physics.Raycast(_facade.Transform.position, Vector3.down,
        out RaycastHit hit, Mathf.Infinity, _config.CanvasLayer))
      {
        var canvas = hit.collider.GetComponent<Renderer>();
        if (!TryCopyFromRenderTexture(canvas.material.mainTexture))
          return;

        Vector3 localScale = 10 * canvas.transform.localScale;
        var canvasSize = new Vector2(localScale.x, localScale.z);
        Vector3 forward = _facade.Transform.forward;
        Vector2 right = new Vector2(_facade.Transform.right.x, _facade.Transform.right.z);
        ...
        _facade.Transform.forward = forward;
      }
    }

    private Vector2 WorldToUVDirection(Renderer canvas, Vector3 worldDirection)
    {
      Vector3 localDirection = canvas.transform.InverseTransformDirection(worldDirection);
      Vector2 localTexturePoint = new Vector2(localDirection.x, localDirection.z);

      return -localTexturePoint;
    }

    private bool TryCopyFromRenderTexture(Texture texture)
    {
    ...
    private Vector2 FindBlackPixel(Vector2 pixelStart, Vector2 pixelEnd, Vector2 textureForward, float pixelWidth, 
      Vector2 right, Texture2D texture) 
    {
      bool isPixelFound = false;
      for (Vector2 i = pixelStart; Vector2.Distance(i, pixelEnd) >= 1 && !isPixelFound; i += textureForward)
      {
        for (float j = -pixelWidth / 2; j < pixelWidth / 2 && !isPixelFound; j++)
        {
          Vector2 t = i + right * j;
    ...
    private Vector2 ComputeGradient(Texture2D texture, Vector2 uv, float uvWindowSize = 0.03f)
    {
      float u = uv.x;
      float v = uv.y;
      ...
      float halfWindowSize = uvWindowSize / 2;
      ...
      for (float du = uMin; du <= uMax; du += uvWindowSize / 3)
      {
        for (float dv = vMin; dv <= vMax; dv += uvWindowSize / 3)
        {
          Color left = texture.GetPixel((int)(du * width), (int)(dv * height));
          Color right = texture.GetPixel((int)((du + uvWindowSize / 3) * width), (int)(dv * height));
          Color bottom = texture.GetPixel((int)(du * width), (int)(dv * height));
          Color top = texture.GetPixel((int)(du * width), (int)((dv + uvWindowSize / 3) * height));
